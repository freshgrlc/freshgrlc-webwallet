...
export const AutoPaymentInfo: React.FC<IProps> = ({autopaymentConfig, addHeader, id, deleteCallback}) => {
    const type: TransactionType = autopaymentConfig.transaction.type === 'standard' ? 'standard' :
                                  autopaymentConfig.transaction.amountToKeep && autopaymentConfig.transaction.amountToKeep > 0.0 ? 'keep-amount' : 'everything';
    return (
        <> 
            {addHeader ? (
                <TableHeaderRow>
            ...
            <TableRow>
                ...
                {deleteCallback !== undefined ? (
                    <div><input type='button' value='Delete' onClick={() => deleteCallback(id)} /></div>
                ) : (<></>)}
            </TableRow>
        </>
    );
